...
```bash
json-server db.txt --static ./some-other-dir
```
...
```bash
$ json-server --watch db.txt --port 3004
```
...
```bash
json-server db.txt --routes routes.json
```
...
```bash
json-server db.txt --middlewares ./hello.js
json-server db.txt --middlewares ./first.js ./second.js
```
